...
void  OSIntEnter (void)
{
    if (OSRunning == OS_TRUE) {
        if (OSIntNesting < 255u) {
            OSIntNesting++;                      /* Increment ISR nesting level   有中断发生禁止任务调度                     */
        }
    }
}
/*$PAGE*/
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        ...
        OSStartHighRdy();                            /* Execute target specific code to start task OSStartHighRdy汇编写的,触发PendSV异常,PendSV中断服务程序中进行任务切换    */
    }
}
/*$PAGE*/
/*
